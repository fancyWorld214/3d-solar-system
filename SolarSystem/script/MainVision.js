export default class MainVision {
  ...
  setUp(engine, scene, canvas, camera) {
    ...
    this.sun = BABYLON.Mesh.CreateSphere(
      "Sun",
      1.3927 * diameterScale,
      0.65,
      scene
    );
    this.earth = BABYLON.Mesh.CreateSphere(
      "Earth",
      0.12756 * diameterScale,
      0.3,
      scene
    );
    ...
    //Bonusteil:2
    this.mars = BABYLON.Mesh.CreateSphere(
      "Mars",
      this.mars_diameter,
      0.25,
      scene
    );
    this.jupiter = BABYLON.Mesh.CreateSphere(
      "Jupiter",
      this.jupiter_diameter,
      0.35,
      scene
    );
    this.neptune = BABYLON.Mesh.CreateSphere(
      "Neptune",
      this.neptune_diameter,
      0.37,
      scene
    );
    ...
    this.moon_local_pos = new BABYLON.Vector3(-1.0 * this.moon_distance, 0, 0);
    ...
    scene.beforeRender = function () {
      ...
      var elapsed_t = time - this.startTime; // milliseconds since start
      var delta_t = this.lastTime - time; // milliseconds since last frame
      this.lastTime = time;
      ...
      var earthSpeed = ((elapsed_t % einUmlauf) * 360) / einUmlauf;
      var radians = (earthSpeed * Math.PI) / 180;
      earth.position.x = Math.cos(radians) * this.earthOrbitRadius;
      ...
      this.moon.position.x =
        Math.cos(moonradians) * this.moonOrbitRadius + this.earth.position.x;
      this.moon.position.z =
        Math.sin(moonradians) * this.moonOrbitRadius + this.earth.position.z;
  ...
  setUpCameraMovingPattern(camera, scene) {
    ...
    function moveCameraForward() {
      ...
      camera.position = new BABYLON.Vector3(
        cur_position.x + this.cameraSpeed * direction.x,
        cur_position.y + this.cameraSpeed * direction.y,
        cur_position.z + this.cameraSpeed * direction.z
      );
    }
    ...
    function moveCameraLeft() {
      ...
      var cur_position = camera.position;
      camera.position = new BABYLON.Vector3(
        cur_position.x + this.cameraSpeed * sideDirection.x,
        cur_position.y + this.cameraSpeed * sideDirection.y,
        cur_position.z + this.cameraSpeed * sideDirection.z
      );
